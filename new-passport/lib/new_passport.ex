defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, counter_time} <- enter_building(now),
         {:ok, manual_func} <- find_counter_information(now),
         counter <- manual_func.(birthday),
         {:ok, checksum} <- stamp_form(counter_time, counter, form),
         passport_number <- get_new_passport_number(counter_time, counter, checksum) do
      {:ok, passport_number}
    else
